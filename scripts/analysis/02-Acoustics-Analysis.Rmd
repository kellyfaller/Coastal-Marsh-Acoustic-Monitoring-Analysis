...
params:
  ...
  aru_locations:    "../Metadata/Acoustic Project Sites.csv"            # Site/location metadata
  recording_metadata: "../Metadata/Recording_Metadata.csv"
  arbimon_pm_results: "C:/Users/kelly/Documents/Master's Project Files/Arbimon Backups/salt-marsh-restoration-soundscape-project_2025-08-12_export/pattern_matching_rois.0001.csv"
  arbimon_species_metadata: "C:/Users/kelly/Documents/Master's Project Files/Arbimon Backups/salt-marsh-restoration-soundscape-project_2025-08-12_export/species.0001.csv"
  # Outputs
  output_dir: "C:/Users/kelly/Documents/GitHub/Soundscape-Analysis-Rare-Bird-Detection/Outputs"   # Where results should be written
  use_leaflet_map: true       # set FALSE to skip the map if you don't want it
  # Acoustic index generation parameters
  min_freq_hz: 300          # For acoustic index generation. Decide what this will be for your project
  max_freq_hz: 10000        # For acoustic index generation. Decide what this will be for your project
  db_threshold: -35         # For ADI specifically
  session_length: 900       # Recording length in seconds. Adjust for different duty cycle set-ups. This is for a 15 min recording during a 15 min on 45 min off duty cycle.
  cores: 10                 # This is the amount of computational power that you are using in your computer. Check your computer's settings to see how many cores you have.
  # Optional image overlay
  sparrow_image_path: "../Images/SALS.png" 
  # Map settings
  site_crs: 4326 # If in state plane or other projected coords, set that EPSG (e.g., 6527).
  map_center_lng: -75.15925
  map_center_lat: 39.045748
  # ======================================
---

```{r Clear, warning=FALSE, message=FALSE}
# Clear environment for a clean workspace.
rm(list=ls())
```

```{r setup, include=FALSE}
# Reproducible knitr defaults
...
# Resolve paths safely on any OS
fp <- function(...){ normalizePath(file.path(...), mustWork = FALSE) }

dir.create(params$output_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(fp(params$output_dir, "Acoustic_Index"), showWarnings = FALSE, recursive = TRUE)

paths <- list(
  wav_dir               = fp(params$wav_dir),
  aru_locations         = fp(params$aru_locations),
  recording_metadata    = fp(params$recording_metadata),
  arbimon_pm_results    = fp(params$arbimon_pm_results),
  arbimon_species_meta  = fp(params$arbimon_species_metadata),
  output_dir            = fp(params$output_dir),
  sparrow_image_path    = fp(params$sparrow_image_path)
)

min_f   <- as.numeric(params$min_freq_hz)
max_f   <- as.numeric(params$max_freq_hz)
db_thr  <- as.numeric(params$db_threshold)
sess_s  <- as.integer(params$session_length)
cores   <- as.integer(params$cores)

# Optional parquet precheck if you ever add it later
...
aru_locations <- readr::read_csv(params$aru_locations, show_col_types = FALSE)
stopifnot(nrow(aru_locations) > 0)
```

Arbimon
```{r Loading Arbimon, include=FALSE}
recording_metadata <- vroom::vroom(paths$recording_metadata, show_col_types = FALSE)
pm_results         <- vroom::vroom(paths$arbimon_pm_results, show_col_types = FALSE)
species_metadata   <- vroom::vroom(paths$arbimon_species_meta, show_col_types = FALSE)

stopifnot(nrow(aru_locations) > 0)

# Robust datetime handling from prep output:
# Accept 'datetime_parsed' (preferred) or fallback to 'datetime'
parse_dt <- function(x, tz = "UTC") {
  if (inherits(x, "POSIXct")) return(with_tz(x, tz))
  x_norm <- if (is.character(x)) sub("Z$", "+00:00", x) else x  # handle trailing Z
  try_order <- function(fun, ...) suppressWarnings(fun(...))
  out <- try_order(ymd_hms, x_norm, tz = tz, quiet = TRUE)
  if (all(is.na(out))) out <- try_order(mdy_hms, x_norm, tz = tz, quiet = TRUE)
  if (all(is.na(out))) out <- try_order(ymd_hm,  x_norm, tz = tz, quiet = TRUE)
  if (all(is.na(out))) out <- try_order(mdy_hm,  x_norm, tz = tz, quiet = TRUE)
  if (all(is.na(out))) out <- try_order(ymd,     x_norm, tz = tz, quiet = TRUE)
  if (all(is.na(out))) out <- try_order(mdy,     x_norm, tz = tz, quiet = TRUE)
  out
}

dt_col <- dplyr::case_when(
  "datetime_parsed" %in% names(recording_metadata) ~ "datetime_parsed",
  "datetime"        %in% names(recording_metadata) ~ "datetime",
  TRUE ~ NA_character_
)
if (is.na(dt_col)) stop("recording_metadata needs a datetime column (datetime_parsed or datetime).")

recording_metadata <- recording_metadata %>%
  mutate(
    datetime_parsed = if (dt_col == "datetime_parsed") .data[[dt_col]] else parse_dt(.data[[dt_col]], tz = "UTC"),
    Year            = if (!"Year" %in% names(.)) year(datetime_parsed) else Year,
    Julian_Day      = if (!"Julian_Day" %in% names(.)) yday(datetime_parsed) else Julian_Day
  )

# Optional 'name' fallback
if (!"name" %in% names(recording_metadata)) {
  nm <- intersect(c("site","station","recorder","Site","Station","Recorder"), names(recording_metadata))[1]
  if (!is.na(nm)) recording_metadata$name <- recording_metadata[[nm]]
}

# Quick sanity prints
message(glue("recording_metadata: {nrow(recording_metadata)} rows, {ncol(recording_metadata)} cols"))
rng <- range(recording_metadata$datetime_parsed, na.rm = TRUE)
message(glue("Recording datetime range: {rng[1]} -> {rng[2]}"))
```
